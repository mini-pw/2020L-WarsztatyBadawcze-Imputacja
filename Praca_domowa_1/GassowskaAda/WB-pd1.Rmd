...
Będziemy próbowali przewidzień czy dany klient dostanie kretyd, czy nie. Po zmapowaniu na liczby, klasa 1 oznacza że osoba nie dostanie kredytu, a klasa 2 - że nie. Wszystkie kolumny jesteśmy w stanie zamienić na numeryczne - tak aby model działał dobrze. 
...
df_train[,c(1,4,5,6,7)] <- lapply(df_train[,c(1,4,5,6,7)], function(x) {
  replace(x, is.na(x), mode(na.omit(x)))})
df_train[,c(2,14)] <- lapply(df_train[,c(2,14)], function(x) {
...
id_test <- setdiff(1:n, id_train)
df_test <- data[id_test, c(cechy, klasa)]
df_test[,c(1,4,5,6,7)] <- lapply(df_test[,c(1,4,5,6,7)], function(x) {
  replace(x, is.na(x), mode(na.omit(x)))})
df_test[,c(2,14)] <- lapply(df_test[,c(2,14)], function(x) {
  replace(x, is.na(x), mean(na.omit(x)))})
...
Wszystkie metody wypadły podobnie ok. 0.7, najwyższy dla usuwania kolumn. Jest to niespodziewany wynik. Może po prostu wszystkie użyte przeze mnie metody imputacji są równie złe. 